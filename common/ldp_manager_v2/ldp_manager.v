...
//                       buffers in host RAM
//
// 14-Jun-26  DWW     4  Integrated with new ldp_buffer module
//=============================================================================
...
(
    ...
    // An input stream of frame-status (i.e, frame-error) bits
    input[7:0]     axis_ferr_tdata,
    input          axis_ferr_tvalid,
    output reg     axis_ferr_tready,
...
always @(posedge clk) begin
    ...
    // This strobes high for one cycle at a time to advance the "axis_ferr"
    // FIFO that carries the frame error-status for each frame
    axis_ferr_tready <= 0;
    ...
    if (resetn == 0) begin
        ...
        WSM_WRITE_MD11:
            if (M_AXI_WVALID & M_AXI_WREADY) begin
                axis_ferr_tready <= 1; // Advance the axis_ferr FIFO
